...
namespace klaas
{
	typedef enum {EMPTY, WHITE, BLACK, WHITEQUEEN, BLACKQUEEN} state; // The different states a field can have
	...
	public:
		Field() : d_state(EMPTY), d_number(0), d_lock(false), d_map(0)
		{}
		Field(state st, unsigned number) : d_state(st), d_number(number), d_lock(false), d_map(d_number)
		{}
