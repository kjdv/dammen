        ...
        if(setup[i].getstate() == BLACK) {
            if(setup[i].map().canr3() && setup[setup[i].map().fieldr3()].getstate() == WHITE) {
                if(setup[i].map().canr4()) {
                    ...
                    ret += LOCKVALUE;
            } else if((!setup[i].map().canr3() && setup[i].map().canr4()) && setup[setup[i].map().fieldr4()].getstate() == WHITE)
                ret += LOCKVALUE;
            ...
            if(setup[i].map().canr1() && setup[setup[i].map().fieldr1()].getstate() == BLACK) {
                if(setup[i].map().canr2()) {
                    ...
                    ret -= LOCKVALUE;
            } else if((!setup[i].map().canr1() && setup[i].map().canr2()) && setup[setup[i].map().fieldr2()].getstate() == BLACK)
                ret -= LOCKVALUE;
        }
    }
